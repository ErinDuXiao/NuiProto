...
export type DragState = {
  ...
  x: number;
  /**
   * 描画する足元の y。ドラッグ中は**常に** `rowY(shelfRow)`（棚板の上面ライン）で、
   * 持ち上げの 120ms の途中でも段と段の間には来ない。
   * 例外は Drop 後の滑走中だけ（`settling`。仕様6.3 が滑らせろと言っている）。
   */
  y: number;
  /** いま乗っている段 */
  shelfRow: number;
 ...
 * **「どの段を選ぶかの基準」** に入れているため。120ms のランプも
 * **段を選ぶ基準にしか掛けない** — 描く y にランプを掛けると、その 120ms の
 * 間だけ足元が段と段の間に浮き、同じフレームで影（`rowY(pose.row)` に出る）と
 * 胴体が別の高さに居ることになる。仕様6.1 の「常に」はそれを許さない。
 * ランプは「段の選択が 120ms かけて上へ寄る」形で表れる。掴んだ瞬間は
 * `e = 0` で選ばれる段が現在の段そのものなので、`rowY(row)` は足元の
 * 位置と一致し、飛び上がりも起きない。
 *
 * **足元と指の差について（無条件の保証ではない）。**
 * 段の内側では、吸着が最大で段間隔の半分（102/2 = 51px）だけ下へずれるので、
 * 足元が指より下に来るのは高々 51 − 46 = **5px**。ここまでは胴（72px）の
 * 最下部であって顔には届かない。
 *
 * **しかし最上段より上ではこの上界は成り立たない。** `rowFromY` は
 * `rowY[0] = 214` にクランプするので、指が 214 より上がった分だけ差は
 * そのまま開く（指 y=180 で 34px、y=150 で 64px、y=100 で 114px）。
 * 40px を超えると指は顔に掛かる。
 *
 * **これは持ち上げ量では閉じられない。** 最上段の上に乗せる棚板が無い以上、
 * どんな `DRAG_LIFT_PX` にしても指だけが先に上がる。閉じるには
 * 「棚板に吸着する」（仕様6.1 のもう半分）を捨てて空中に浮かせるしかない。
 * よって**この帯は受け入れる**。プレイヤーの狙いは「最上段に置く」であり、
 * そのとき指は 214 付近にあって差はほぼ 0 になる。214 より上は
 * 「行き過ぎ」の領域で、そこでも配置自体は最上段のまま正しく決まる。
 * `useDragPlacement.test.tsx` がこの帯を含めて掃引し、
 * 「差が開くのは最上段にクランプされたぶんだけ」を機械で固定している。
 */
function poseOf(live: Live, t: number): { x: number; y: number; row: number } {
  ...
  // 描く足元は**必ず**選んだ段の上面ライン。ここに `e` を掛けて
  // anchor から補間すると、ランプ中だけ棚板から浮く（上の docstring）。
  return { x, y: rowY(row), row };
}
